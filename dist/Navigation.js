...
var Navigation = /*#__PURE__*/function () {
  ...
  _createClass(Navigation, [{
    ...
    value: function removeNoJs() {
      this.menuElements.map(function (menu) {
        // console.log(menu);
        var listItems = Array.from(menu.querySelectorAll('.no-js')); // console.log({listItems});
    ...
    value: function toggleCurrentTopLevelItemClass(target) {
      var topLevelItems = this.menuElements.map(function (el) {
        return Array.from(document.querySelectorAll("#".concat(el.id, " > li")));
      }).flat();
    ...
    value: function manageSubmenuState(target) {
      var buttons = this.menuElements.map(function (el) {
    ...
    value: function setMenuEventListeners() {
      ...
      this.menus.map(function (menu) {
        ...
        var menuEl = document.getElementById(menu.menuId);

        if (menu.click) {
          listeners.push('click');
          var subMenuList = Array.from(menuEl.querySelectorAll('.am-submenu-list'));
        ...
        for (var i = 0; i < listeners.length; i++) {
          menuEl.addEventListener(listeners[i], function (evt) {
    ...
    value: function setDocumentEventListeners(target) {
      ...
      if (target.getAttribute('aria-expanded') === 'true') {
        ...
        document.addEventListener('focusin', function (_ref3) {
          var target = _ref3.target;

          _this2.menuElements.map(function (el) {
    ...
    key: "clickHandler",
    value: function clickHandler(_ref4) {
    ...
    key: "focusInHandler",
    value: function focusInHandler(_ref5) {
      var _this3 = this;

      var target = _ref5.target,
          relatedTarget = _ref5.relatedTarget;
      var topItem = this.toggleCurrentTopLevelItemClass(target);
      this.menuElements.map(function (el) {
        console.log({
          elContains: el.contains(relatedTarget),
          topContains: topItem.contains(relatedTarget)
        });
    ...
    value: function init() {
      ...
      this.menuElements = this.menus.map(function (_ref6) {
        var menuId = _ref6.menuId;
